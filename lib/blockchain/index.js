...
class Blockchain {
    ...
    getDifficulty(index) {
        const currentDifficulty = Config.pow.getDifficulty(this.blocks, index);
        
        if (index < Config.DIFFICULTY_ADJUSTMENT_INTERVAL) {
            return currentDifficulty; 
        }

        const lastBlocks = this.blocks.slice(-Config.DIFFICULTY_ADJUSTMENT_INTERVAL);
        const totalTime = lastBlocks[lastBlocks.length - 1].timestamp - lastBlocks[0].timestamp;
        const averageTime = totalTime / lastBlocks.length;

        const targetTime = Config.TARGET_BLOCK_TIME * Config.DIFFICULTY_ADJUSTMENT_INTERVAL; 
        let newDifficulty = currentDifficulty;

        console.info(`开始第${index}个区块的难度调整:`);
        console.info(`预期时间: ${targetTime}秒, 实际时间: ${totalTime}秒`);
        console.info(`当前难度: ${currentDifficulty}`);

        if (averageTime < targetTime / Config.MAX_ADJUSTMENT_RATIO) {
            // 如果时间太短, 提高难度
            newDifficulty = currentDifficulty / Config.MAX_ADJUSTMENT_RATIO;
            console.info(`时间太短，提高难度至: ${newDifficulty}`);
        } else if (averageTime > targetTime * Config.MAX_ADJUSTMENT_RATIO) {
            // 如果时间太长, 降低难度
            newDifficulty = currentDifficulty * Config.MAX_ADJUSTMENT_RATIO;
            console.info(`时间太长，降低难度至: ${newDifficulty}`);
        } else {
            // 正常调整
            newDifficulty = currentDifficulty * (averageTime / targetTime);
            console.info(`正常调整难度至: ${newDifficulty}`);
        }

        return Math.max(Math.floor(newDifficulty), 1); // 确保难度至少为1
    }

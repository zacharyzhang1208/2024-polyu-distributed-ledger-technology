...
class Blockchain {
    ...
    getDifficulty(index) {
        ...
        if (index < Config.DIFFICULTY_ADJUSTMENT_INTERVAL) {
            return currentDifficulty; // Not enough blocks to adjust difficulty
        }
        ...
        if (averageTime < Config.TARGET_BLOCK_TIME) {
            return Math.max(currentDifficulty - 1, 1); // Increase difficulty
        } else if (averageTime > Config.TARGET_BLOCK_TIME) {

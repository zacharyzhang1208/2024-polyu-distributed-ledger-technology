...
class HttpServer {
    constructor(node, blockchain, operator, miner) {
        ...
        this.app.use(cors({
            ...
            allowedHeaders: ['Content-Type', 'Authorization', 'password'] // 确保包含password header
        }));
        ...
        this.app.post('/operator/wallets/:walletId/attendance', (req, res) => {
            ...
            try {
                ...
                if (!operator.checkWalletPassword(walletId, passwordHash)) throw new HTTPError(403, `Invalid password for wallet '${walletId}'`);
                // 创建考勤交易
                let newTransaction = operator.createAttendanceTransaction(walletId, req.body.fromAddress, req.body.toAddress, req.body.studentId, req.body.activityId, req.body.verifyCode);

...
class Node {
    ...
    checkReceivedBlocks(blocks) {
        ...
        // 计算当前链和收到的链的累计难度
        const currentDifficulty = Block.calculateCumulativeDifficulty(this.blockchain.getAllBlocks());
        const receivedDifficulty = Block.calculateCumulativeDifficulty(receivedBlocks);

        // 如果收到的链累计难度不大于当前链,不做处理
        if (receivedDifficulty <= currentDifficulty) {
            console.info('Received blockchain is not more difficult than current blockchain. Do nothing');
            return false;
        }

        console.info(`Blockchain possibly behind. Current cumulative difficulty: ${currentDifficulty}, Received cumulative difficulty: ${receivedDifficulty}`);
